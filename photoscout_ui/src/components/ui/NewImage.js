...
function UnannotatedImage(image, map, setHighlightedObject, new_width, addImage, img_dir, objs, imgSaved, handleSavedImages, desc, setSelectedObject, selectedObject, isFace, handleTaggingTextChange, handleTaggingTextSubmit, isTagged, tags) {
    ...
    return <Box>
        ...
        <ImageMapper src={image.replace("photoscout_ui/public/", "./")} map={map} onMouseLeave={(area, index) => setHighlightedObject(null)} onClick={(area, index) => setSelectedObject(index)} onMouseEnter={(area, index) => setHighlightedObject(index)} toggleHighlighted={true} stayHighlighted={true} width={new_width} />
